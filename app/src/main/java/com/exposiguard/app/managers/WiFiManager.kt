...
class WiFiManager @Inject constructor(
    ...
    fun startWifiScan(): Boolean {
        return try {
            ...
            if (!hasLocationPermission() && !hasNearbyWifi) {
                android.util.Log.w("WiFiManager", "No hay permisos de ubicación para escanear WiFi")
                return false
            }

            if (!wifiManager.isWifiEnabled) {
                android.util.Log.w("WiFiManager", "WiFi está deshabilitado, no se puede escanear")
                return false
            }

            val success = try { wifiManager.startScan() } catch (_: Throwable) { false }
            if (success) {
                android.util.Log.d("WiFiManager", "WiFi scan started successfully")
            } else {
                android.util.Log.w(
                    "WiFiManager",
    ...
    fun getCarrierNetworkInfo(): String {
        ...
        if (availableNetworks.isEmpty()) {
            return "📡 No se detectaron redes de operadores móviles\n• Verifica permisos de ubicación\n• Asegúrate de que los datos móviles estén activados"
        }
        ...
        val base = """
        📡 Redes de Operadores Móviles Detectadas:
        • $networksText

        💡 Se muestran SIM cards instaladas y redes disponibles en el área
        """.trimIndent()

        return if (onlySimDetected) {
            base + "\n\nℹ️ Android puede limitar la detección de otras operadoras cercanas en algunos dispositivos. Activa la ubicación y los datos móviles, y prueba al aire libre para mejores resultados."
        } else base
    }
