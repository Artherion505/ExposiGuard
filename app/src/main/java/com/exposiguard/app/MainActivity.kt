...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Forzar idioma inglés por defecto si no hay ninguno guardado
        val currentLanguage = com.exposiguard.app.utils.LocaleHelper.getPersistedLanguage(this)
        if (currentLanguage == "en") {
            // Si ya está en inglés, asegurarse de que se aplique
            com.exposiguard.app.utils.LocaleHelper.setLocale(this, "en")
        }
    ...
    private fun handleSinglePermissionResult(isGranted: Boolean) {
        ...
        if (isGranted) {
            Toast.makeText(this, getString(R.string.permission_granted, currentGroup.name), Toast.LENGTH_SHORT).show()
        } else {
            Toast.makeText(this, getString(R.string.permission_denied, currentGroup.name), Toast.LENGTH_SHORT).show()
        }
        ...
        try {
            ...
            val message = when {
                e.message?.contains("No Activity found") == true ->
                    ...
                    getString(R.string.health_connect_open_error)
            }
    ...
    private fun showPermissionSummary() {
        ...
        when {
            totalMissing == 0 -> {
                Toast.makeText(this, getString(R.string.permissions_all_configured), Toast.LENGTH_LONG).show()
            }
            healthMissing.isNotEmpty() -> {
                val message = getString(R.string.permissions_setup_complete_with_health, allMissingPermissions.size, healthMissing.size)
                Toast.makeText(this, message, Toast.LENGTH_LONG).show()
            }
            else -> {
                val message = getString(R.string.permissions_setup_complete_optional, allMissingPermissions.size)
                Toast.makeText(this, message, Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    private fun handlePermissionResults(permissions: Map<String, Boolean>) {
        ...
        when {
            totalGranted == totalRequested -> {
                Toast.makeText(this, getString(R.string.permissions_all_granted, currentGroup.name), Toast.LENGTH_SHORT).show()
            }
            totalGranted > 0 -> {
                Toast.makeText(this, getString(R.string.permissions_partial, currentGroup.name, totalGranted, totalRequested), Toast.LENGTH_SHORT).show()
            }
            else -> {
                Toast.makeText(this, getString(R.string.permissions_all_denied, currentGroup.name), Toast.LENGTH_SHORT).show()
            }
        }
    ...
    /**
     * Aplicar el idioma guardado antes de que se cree el contexto de la actividad.
     * Esto asegura que la app inicie en el idioma correcto desde el primer lanzamiento.
     */
    override fun attachBaseContext(newBase: Context) {
        // Obtener el idioma guardado, o usar "en" como idioma por defecto si no hay ninguno
        val languageCode = com.exposiguard.app.utils.LocaleHelper.getPersistedLanguage(newBase)
        val context = com.exposiguard.app.utils.LocaleHelper.setLocale(newBase, languageCode)
